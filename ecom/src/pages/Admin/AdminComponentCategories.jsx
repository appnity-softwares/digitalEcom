...
import {
    Search, Plus, Edit, Trash2, X, Star, Eye, Layers, Palette
} from 'lucide-react';
import {
    useComponentCategories,
    useCreateComponentCategory,
    useUpdateComponentCategory,
    useDeleteComponentCategory
} from '../../hooks/useQueries';
import { useToast } from '../../context/ToastContext';

const AdminComponentCategories = () => {
    const { showToast } = useToast();
    const [showModal, setShowModal] = useState(false);
    const [editingCategory, setEditingCategory] = useState(null);

    // Queries
    const { data: categoriesResponse, isLoading } = useComponentCategories('all');

    // Mutations
    const createMutation = useCreateComponentCategory();
    const updateMutation = useUpdateComponentCategory();
    const deleteMutation = useDeleteComponentCategory();

    const categories = categoriesResponse?.data || [];

    // State for Search (Filtered locally since payload is small)
        ...
        <div className="p-8">
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-muted-foreground">Manage component categories and organization</p>
                </div>
                <button
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                                ...
                                                <span className="flex items-center gap-1">
                                                    ...
                                                    {category._count?.components || 0} components
                                                </span>
                                            </div>
                                        </div>
                                    </div>
...
export default AdminComponentCategories;
