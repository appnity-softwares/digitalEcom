...
const TemplateGrid = ({ items, limit, bento = false }) => {
  ...
  return (
    <section className="w-full bg-background px-6 pb-20 font-sans">
      <div className="max-w-7xl mx-auto flex flex-col gap-12">
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 ${bento ? 'lg:grid-cols-4 auto-rows-[minmax(300px,auto)] grid-flow-dense' : 'lg:grid-cols-3'} gap-6`}>
          {templates.map((template, index) => {
            // Bento Grid Logic (Only if enabled)
            let spanClasses = "md:col-span-1 md:row-span-1";

            if (bento) {
              const isLarge = index === 0 || index % 7 === 0; // 2x2
              const isWide = index % 5 === 0 && !isLarge; // 2x1
              const isTall = index % 3 === 0 && !isLarge && !isWide; // 1x2

              if (isLarge) spanClasses = "md:col-span-2 md:row-span-2";
